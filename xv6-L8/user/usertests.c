...
copyout(char *s)
{
  uint64 addrs[] = { 0LL, 0x80000000LL, 0x3fffffe000, 0x3ffffff000,
                     0x4000000000,
                     0xffffffffffffffff };
...
fourteen(char *s)
{
  ...
  if(fd < 0){
    printf("%s: create 123456789012345/123456789012345/"
...
sbrkmuch(char *s)
{
  ...
  p = sbrk(amt);
  if (p != a) {
    printf("%s: sbrk test failed to grow big address space; "
           "enough phys mem?\n", s);
    exit(1);
  }
  ...
  *lastaddr = 99;
  ...
  c = sbrk(0);
  if(c != a - PGSIZE){
    printf("%s: sbrk deallocation produced wrong address, "
           "a %p c %p\n", s, a, c);
    exit(1);
  }
